...
blueLed = Pin(13, Pin.OUT) #blue LED into pin 14 responds to humidity level
...
#start by reading the sensors
    ...
    #LIGHT LEVEL
    #read the light level from photodiode
    #convert to a percentage, 0 dp
    #print the light level reading
    lightPct = int(lightLevel.read_u16())/655.35
    print(f"Light level is {lightPct}%")

    #HUMIDITY LEVEL
    #read the humidity level from sensor
    #convert to a percentage, 0 dp, flip so 100% is most humid
    #print the light level reading
...
#to respond depending on comparison of reading with threshold
    ...
    if humidityPct < humidityThreshold:
        ...
        print ("Humidity is higher than threshold - turn extractor fan (Yellow LED) on")
