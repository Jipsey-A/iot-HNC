...
tempActionTopic = thingName+'/tempAction'
lightActionTopic = thingName+'/lightAction'
...
heaterBoost = Pin(10, Pin.IN, Pin.PULL_UP) #this is a button
boostActive = False #a boolean to keep tabs on and communicate the boost status
boostEndTime = 0 #this updates in boostStart() to tell us what time the boost finishes
lastSensorTime = 0 #this tells us how long it's been since the last sensor reading
...
def masterCallback(topic, msg):
    ...
    global tempThreshold, lightThreshold, humidityThreshold, light_action, boostActive
...
#let's me know we've received a message and what it says
    #print("Received:", topic, msg) commented out because too many messages from dashboard
...
#update thresholds accordingly & immediately. Uses 'try' and 'except' in case it receives something unexpected in the data field
    ...
    elif "boostButton" in topic:
        try:
            if heaterBoost.value() == 1 and not boostActive:
                print("Boost button on remote dashboard pressed")
                boostStart()
        except:
            print("Invalid boost button request received")
    elif "lightAction" in topic:
        try:
...
def physicalButton():
    global boostActive
    if heaterBoost.value() == 0 and not boostActive:
        print("Boost button on IoT device pressed")
        boostStart()

#start the heat boost sequence
def boostStart():
    global boostActive, boostEndTime, client, boostTime
    if boostActive == False:
        boostActive = True
        boostEndTime = time.time() + boostTime
        print("Heater Boost activated for 20 seconds")
        client.publish(boostStatusTopic, str(boostActive))

# Check if boost time has expired and handles flashing LED
def boostStatusCheck():
    global boostActive, boostEndTime, client
    
    #if boost time has ended make boostActive false
    if boostActive and time.time() >= boostEndTime:
        boostActive = False
        print("Heater boost ended - returning to normal control")
    #if boost is still active flash the LED
    elif boostActive:
        ms = time.ticks_ms()
        redLed.value((ms // 250) % 2)   # 2 Hz flash
...
while True:
    ...
    boostStatusCheck() #check if boost is in progress
    physicalButton() #check if physical button has been pressed
    
    ms = time.ticks_ms() # reads the time
    #only reads sensors and amends actuators if sensorInterval has passed.
    if time.ticks_diff(ms, lastSensorTime) >= sensorInterval:
        lastSensorTime = ms
        ...
        if boostActive:
            temp_action = 1
            print("Boost mode")
        elif tempC < tempThreshold:
            temp_action = 1
            redLed.value(temp_action)
        ...
        client.publish(tempActionTopic, str(temp_action))
    
        #LIGHT main light represented by yellow LED is controlled by Node red. 
        yellowLed.value(light_action)
        if light_action == 1:
        ...
        client.publish(humidActionTopic, str(humid_action))
